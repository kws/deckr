...
class LocalSubstrate:
    def __init__(
        ...
        *,
        ...
        self._states: dict[str, LocalStateStore] = {}

    def start(self, tg: anyio.abc.TaskGroup) -> None:
        tg.start_soon(self._sweep_state, name="deckr_local_state_sweeper")
    ...
    def state(self, name: str) -> StateStore:
        ...
        if store is None:
            store = LocalStateStore(name=name, buffer_size=self._buffer_size)
    ...
    async def put(
        ...
        *,
        ...
        async with self._lock:
            ...
            self._entries[key] = (entry, state_expires_at(normalized, ttl=ttl))
