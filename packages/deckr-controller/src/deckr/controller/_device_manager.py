...
class DeviceManager:
    ...
    async def handle_command(self, msg: HostMessage) -> None:
        ...
        elif msg_type == SET_KEY_IMAGE:
            from deckr.plugin.graph_wire import graph_from_wire
            from invariant import SubGraphNode
            from invariant.params import ref

            parsed = graph_from_wire(payload.get("graph", {}))
            if parsed:
                ...
                await router.set_key_image(graph)
