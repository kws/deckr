...
from deckr.hw.events import HWSImageFormat
...
class RenderModel:
    ...
    title: str | None = None
    image: str | None = None
    graph: Node | SubGraphNode | None = None
...
def _state_override_to_model(
    ...
    """Build RenderModel from a StateOverride; priority graph > image > title."""
    if override.graph is not None:
        return RenderModel(graph=override.graph)
