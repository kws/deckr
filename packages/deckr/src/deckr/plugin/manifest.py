"""Plugin manifest models. Pydantic-based for dual Python/JSON representation.

Use Manifest.model_validate_json(json_str) to parse Elgato-style JSON (PascalCase).
Use manifest.model_dump_json(by_alias=True) to emit Elgato-compatible JSON.
"""
...
@dataclass
class StateOverride:
    """Per-state render override: title, image URL/data URI, or custom graph."""

    title: str | None = None
    image: str | None = None  # URL or data URI (for image_card recipe)
    graph: Node | SubGraphNode | None = None  # custom SubGraphNode (set_key_image)
...
class Manifest(CamelModel):
    """Plugin manifest. Contains actions with per-state defaults."""
