...
from deckr_python_runtime import cli as cli_mod
from deckr_python_runtime.config import ConfigDocument
...
def test_load_launcher_document_passes_path_to_default_loader(
    ...
    monkeypatch.setattr("deckr_python_runtime.launcher.load_config_document", fake_load_config_document)
...
def test_launch_runs_before_run_hook_and_anyio(
    ...
    monkeypatch.setattr("deckr_python_runtime.launcher.validate_component_configuration", fake_validate)
    monkeypatch.setattr("deckr_python_runtime.launcher.anyio.run", fake_anyio_run)
...
def test_run_document_uses_signal_handler_and_runner(
    ...
    monkeypatch.setattr("deckr_python_runtime.launcher.add_signal_handler", fake_add_signal_handler)
...
def test_run_configured_deckr_uses_public_runtime_and_component_host(
    ...
    document = ConfigDocument(raw={"deckr": {}}, source_path=None, base_dir=Path.cwd())
    monkeypatch.setattr("deckr_python_runtime.launcher.resolve_component_host_plan", lambda doc: plan)
    monkeypatch.setattr("deckr_python_runtime.launcher.Deckr", FakeDeckr)
    monkeypatch.setattr("deckr_python_runtime.launcher.start_components", fake_start_components)
    monkeypatch.setattr("deckr_python_runtime.launcher.anyio.sleep_forever", fake_sleep_forever)
