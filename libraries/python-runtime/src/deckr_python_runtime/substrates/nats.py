...
class NatsSubstrate:
    ...
    async def connect(self) -> None:
        try:
            ...
            raise RuntimeError(
                "NATS substrate requires deckr-python-runtime[nats]."
            ) from exc
    ...
    async def publish(self, message: DeckrMessage) -> None:
        ...
        await self._publish_payload(
            lane_message_subject(message),
            message,
            headers=lane_message_headers(message),
        )
    ...
    async def _publish_payload(
        ...
        *,
        ...
        if self._nc is None:
            raise RuntimeError("NATS substrate is not connected")
        await self._nc.publish(subject, lane_message_payload(message), headers=dict(headers))
    ...
    def _message_from_nats(self, msg) -> DeckrMessage:
        ...
        validate_lane_subject_hint(msg.subject, message)
        validate_lane_headers(getattr(msg, "headers", None), message)
        contract = self._lane_contracts.contract_for(message.lane)
        validate_message_for_contract(message, contract)
        return message
...
class NatsStateStore:
    ...
    async def put(
        ...
        *,
        ...
        try:
            revision = await kv.put(key, state_payload(normalized))
        except Exception as exc:
    ...
    async def create(
        ...
        *,
        ...
        normalized = state_value(value)
        try:
            revision = await kv.create(key, state_payload(normalized))
        except Exception as exc:
